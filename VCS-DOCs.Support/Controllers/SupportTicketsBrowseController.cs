...
public sealed class SupportTicketsBrowseController : ControllerBase
{
	...
	/// <summary>
	/// Закрытые заявки для операторов.
	/// Параметры:
	///   scope: all | mine | unassigned
	///   org: точное совпадение организации
	///   q: поиск по Id/Subject/UserLogin/Organization
	/// </summary>
	[HttpGet("closed")]
	public async Task<IActionResult> Closed([FromQuery] string? scope = "all",
											[FromQuery] string? org = null,
											[FromQuery] string? q = null)
	{
		var me = CurrentUserId();
		...
		var result = rows.Select(x => new
		{
			...
			closedAt = x.UpdatedAt ?? x.CreatedAt,
			assignedUserId = x.AssignedUserId,
			operatorLogin = x.AssignedUserId != null
				? _db.Users.AsNoTracking().Where(u => u.Id == x.AssignedUserId).Select(u => u.UserName).FirstOrDefault()
				: x.LastOpLogin
	...
	/// status:
	///   closed -> организации из закрытых
	///   иначе  -> организации из НЕзакрытых
	/// </summary>
	[HttpGet("orgs")]
	public async Task<IActionResult> Orgs([FromQuery] string? status = null)
	{
		var wantClosed = string.Equals(status?.Trim(), "closed", StringComparison.OrdinalIgnoreCase);

		var orgs = await (
				from t in _db.SupportTickets.AsNoTracking()
				where wantClosed ? t.Status == "closed" : t.Status != "closed"
				join u in _db.Users.AsNoTracking() on t.OwnerUserId equals u.Id into uJoin
				from u in uJoin.DefaultIfEmpty()
