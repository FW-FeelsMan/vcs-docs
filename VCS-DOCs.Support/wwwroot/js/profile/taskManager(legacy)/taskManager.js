...
window.taskManager = (function () {
                            ...
                            cancelBtn.addEventListener("click", async () => {
                                ...
                                try {
                                    ...
                                    if (task.taskKey.startsWith("upload_")) {
                                        response = await fetch("/api/upload/cancel", {
                                            method: "POST",
                                            headers: { "Content-Type": "application/json" },
                                            body: JSON.stringify({ hash: task.taskKey.replace("upload_", "") })
                                        });

                                        if (!response.ok) throw new Error(`HTTP ${response.status}`);
                                    }

                                    task.statusClass = "canceled";
                                    task.statusText = "Отменено";
                                    task.cancelable = false;

                                    render();
                                } catch (err) {
                                    console.error("Ошибка отмены задачи:", err);
                                    cancelBtn.textContent = "Ошибка";
                                    setTimeout(() => {
    ...
    function removeTask(task, toBlacklist = false) {
        ...
        const containers = getAllTaskContainers();
        const el = containers.map(c => c.querySelector(`.task-status[data-taskkey="${task.taskKey}"]`)?.closest(".task-card")).find(Boolean);

        if (toBlacklist) removedTasks.set(task.taskKey, true);
        if (el) {
