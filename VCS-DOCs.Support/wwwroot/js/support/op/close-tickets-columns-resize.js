// wwwroot/js/support/op/close-tickets-columns-resize.js
// Close tickets columns resize (same stable logic as open/accounts)

(() => {
    if (window.__opCloseTicketsColResizeInit) return;
    window.__opCloseTicketsColResizeInit = true;

    const ROOT_ID = "op-close-tickets";
    const TABLE_ID = "opCloseTicketsTable";

    const STORAGE_KEY = "opCloseTicketsTable.colWidths.v2";
    const MIN_COL_PX = 70;
    ...
    function ensureHoverCssOnce() {
        // уже инжектится в open, но на всякий — одно и то же id
    ...
    function findGripFromEvent(e, table) {
        ...
        if (nearRight >= 0 && nearRight <= GRIP_PX) return idxCell; // last allowed
    ...
    window.initOpCloseTicketsColResize = initOnce;
})();
