(() => {
    ...
    function mockThread(id, fromId) {
        const now = Date.now(), base = now - 60 * 60 * 1000;
        const closed = fromId === 'closed_tickets' || /zx$/i.test(id);
        return {
            status: closed ? 'Закрыта' : 'Открыта',
            created: new Date(base),
            updated: new Date(base + 25 * 60 * 1000),
            messages: [
                { role: 'user', at: new Date(base + 0 * 60 * 1000), text: 'Здравствуйте! Нужна помощь.' },
                { role: 'operator', at: new Date(base + 10 * 60 * 1000), text: 'Добрый день! Уточните, пожалуйста, детали.' }
            ]
        };
    }

    function renderMsg(m) {
        const who = m.role === 'operator' ? 'Оператор' : 'Вы';
    ...
    window.initUTicketThread = async function (panel) {
        ...
        const fromId = ds.from || 'open_tickets';
