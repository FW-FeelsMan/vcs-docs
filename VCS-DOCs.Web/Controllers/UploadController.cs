...
public sealed class UploadController : ControllerBase
{
		...
		CancellationToken ct)
	{
		...
		if (act is not null && !string.Equals(act.FileHash, hash, StringComparison.OrdinalIgnoreCase))
		{
			...
			if (ageSec <= FreshSeconds && !act.Stopped)
			{
				...
				{
					status = "busy",
					message = "Идёт другая загрузка. Дождитесь окончания или нажмите «Заново» в текущей."
				});
			}
		}
		...
		if (!r.ok)
		{
			...
			if (r.message is "av_timeout" or "av_unavailable")
		...
		return Ok(new
		{
			nextExpectedIndex = r.nextExpectedIndex,
			sessionId = r.sessionId,
			completed = r.nextExpectedIndex == totalChunks
		});
	}
	...
	{
		var shortUserId = GetRequiredShortUserIdSafe();
	...
	public async Task<IActionResult> DeleteFile(Guid fileGroupId, int version, CancellationToken ct)
	{
		var shortUserId = GetRequiredShortUserId();
	...
	public async Task<IActionResult> CreateShareLink(
		[FromForm] Guid fileGroupId,
		[FromForm] int version,
		[FromForm] int ttlHours = 168,
	...
	public async Task<IActionResult> PublicDownload(
		[FromQuery] Guid g,
		[FromQuery] int v,
		[FromQuery] long exp,
	...
	[HttpPost("share-db")]
	public async Task<IActionResult> CreateShareDb(
		[FromForm] Guid fileGroupId,
		[FromForm] int version,
		[FromForm] int ttlHours = 168,
		[FromForm] int? maxDownloads = null,
		[FromForm] bool requireAuth = false,
		CancellationToken ct = default)
	{
		if (version <= 0) return BadRequest("invalid version");
		if (ttlHours <= 0 || ttlHours > 24 * 30) ttlHours = 168;
	...
	public async Task<IActionResult> PublicDbDownload([FromRoute] Guid id, CancellationToken ct = default)
	{
		...
		var shortId = GetRequiredShortUserIdSafe();
