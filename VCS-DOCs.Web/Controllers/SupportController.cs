...
public sealed class SupportController : ControllerBase
{
	...
	public SupportController(IConfiguration cfg, IHttpClientFactory http, ILogger<SupportController> log)
	{
		_cfg = cfg;
		_http = http;
		_log = log;
	...
	public async Task<IActionResult> Ticket([FromBody] SupportTicketRequest req, CancellationToken ct)
	{
		...
		if (_cfg.GetValue<bool>("Captcha:Enabled"))
		{
			...
			else if (provider.Equals("ReCaptchaV2", StringComparison.OrdinalIgnoreCase))
			{
				...
				if (!await VerifyRecaptchaV2Async(secret, req.CaptchaToken, ip, ct))
					return BadRequest("captcha_failed");
			}
		}
		...
		_log.LogInformation(
			"Support ticket {TicketId} from {Email}: {Subject}; Code={Code}; Path={Path}; Trace={Trace}",
	...
	public IActionResult NewCaptcha()
	{
		...
		HttpContext.Session.SetString(CaptchaSessionPrefix + id, JsonSerializer.Serialize(data));
		return Ok(new { id });
	}
	...
	{
		var saved = TryGetCaptcha(id);
		if (saved is null)
			return NotFound();

		var svg = RenderCaptchaSvg(saved.Expr);
	...
	private async Task<bool> ValidateLocalCaptchaAsync(string? id, string? answer)
	{
		...
		var key = CaptchaSessionPrefix + id;
		var saved = TryGetCaptcha(id);
	...
	private LocalCaptchaData? TryGetCaptcha(string id)
	{
		var raw = HttpContext.Session.GetString(CaptchaSessionPrefix + id);
		if (string.IsNullOrEmpty(raw))
			return null;

		LocalCaptchaData? saved;
		try
		{
			saved = JsonSerializer.Deserialize<LocalCaptchaData>(raw);
		}
		catch
		{
			return null;
	...
	private static (string expr, string answer) MakeSimpleMath()
	{
		...
		if (rnd.Next(0, 3) == 0 && a > b)
			return ($"{a} - {b}", (a - b).ToString(CultureInfo.InvariantCulture));
	...
	private static string RenderCaptchaSvg(string text)
	{
		...
		foreach (var ch in text)
		{
			...
			var y = 34 + rnd.Next(-2, 3);
			var rot = rnd.Next(-12, 13);
			...
			sb.Append(
				...
				$"fill='#1f2937' stroke='#ffffff' stroke-width='1.2' paint-order='stroke fill' " +
